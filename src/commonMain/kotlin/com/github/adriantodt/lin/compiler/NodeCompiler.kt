...
public class NodeCompiler(private val source: CompiledSourceBuilder = CompiledSourceBuilder()) : NodeVisitor {
    ...
    override fun visitBooleanExpr(node: BooleanExpr) {
        builder.markSection(node) { builder.pushBooleanInsn(node.value) }
    }

    override fun visitBreakExpr(node: BreakExpr) {
        builder.markSection(node) { builder.breakInsn() }
    }

    override fun visitContinueExpr(node: ContinueExpr) {
        builder.markSection(node) { builder.continueInsn() }
    }
    ...
    override fun visitDecimalExpr(node: DecimalExpr) {
        builder.markSection(node) { builder.pushDecimalInsn(node.value) }
    ...
    override fun visitIdentifierExpr(node: IdentifierExpr) {
        builder.markSection(node) { builder.getVariableInsn(node.name) }
    }
    ...
    override fun visitIntegerExpr(node: IntegerExpr) {
        builder.markSection(node) { builder.pushIntegerInsn(node.value) }
    }
    ...
    override fun visitNullExpr(node: NullExpr) {
        builder.markSection(node) { builder.pushNullInsn() }
    }
    ...
    override fun visitStringExpr(node: StringExpr) {
        builder.markSection(node) { builder.pushStringInsn(node.value) }
    }
    ...
    override fun visitThisExpr(node: ThisExpr) {
        builder.markSection(node) { builder.pushThisInsn() }
    }
