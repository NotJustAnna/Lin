...
//        // TODO REDO
//        return when (val value = node.condition) {
//            is BooleanExpr -> if (value.value) node.thenBranch else node.elseBranch ?: NullExpr(node.section)
//            is IntExpr -> if (value.value == 0) node.thenBranch else node.elseBranch ?: NullExpr(node.section)
//            is LongExpr -> if (value.value == 0L) node.thenBranch else node.elseBranch ?: NullExpr(node.section)
//            is FloatExpr -> if (value.value == 0f) node.thenBranch else node.elseBranch ?: NullExpr(node.section)
//            is DoubleExpr -> if (value.value == 0.0) node.thenBranch else node.elseBranch ?: NullExpr(node.section)
//            is NullExpr -> node.elseBranch ?: NullExpr(node.section)
//            is ReturnExpr, is ThrowExpr, is BreakExpr, is ContinueExpr -> value
//            else -> return node
//        }
//    }
...
//            is BooleanExpr -> if (condition.value) LoopNode(node.body, node.section) else NullExpr(node.section)
//            is IntExpr -> if (condition.value != 0) LoopNode(node.body, node.section) else NullExpr(node.section)
//            is LongExpr -> if (condition.value != 0L) LoopNode(node.body, node.section) else NullExpr(node.section)
//            is FloatExpr -> if (condition.value != 0f) LoopNode(node.body, node.section) else NullExpr(node.section)
//            is DoubleExpr -> if (condition.value != 0.0) LoopNode(node.body, node.section) else NullExpr(node.section)
...
//        if (this is MultiNode) return MultiExpr(list, NullExpr(section), section)
//        return MultiExpr(listOf(this), NullExpr(section), section)
//    }
//
